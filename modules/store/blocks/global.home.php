		   ...
		   $catid_search = 0;
			...
			if(count($array_op) > 0)
			{
				...
				if($catid_search > 0)
				{
					if($catid_search > 0)
					{
						...
						if($array_op[0] == 'map' and !empty($array_op[2]))
						{
						    ...
						    $id_tinhthanh = $db->query("SELECT provinceid FROM ".NV_PREFIXLANG . "_" . $module_data. "_province WHERE alias like '". $array_op[2] ."'  ORDER BY weight ASC")->fetchColumn();
						}
						if($array_op[0] == 'map' and !empty($array_op[2]) and !empty($array_op[3]) and $id_tinhthanh > 0 )
						{
						    $id_quanhuyen = $db->query("SELECT districtid FROM ".NV_PREFIXLANG . "_" . $module_data. "_district WHERE provinceid =". $id_tinhthanh ." AND alias like '". $array_op[3] ."'")->fetchColumn();
						}
						if($array_op[0] == 'map' and !empty($array_op[1]) and !empty($array_op[3]) and !empty($array_op[4])  and $id_tinhthanh > 0 and $id_quanhuyen > 0)
						{
							$id_xaphuong = $db->query("SELECT wardid FROM ".NV_PREFIXLANG . "_" . $module_data. "_ward WHERE  districtid =". $id_quanhuyen ." AND alias like '". $array_op[4] ."'")->fetchColumn();
						}
					}
				}
				else
				{
					...
					{
						// TÌM ID TỈNH THÀNH DỰA VÀO ALIAS
					    $id_tinhthanh = $db->query("SELECT provinceid FROM ".NV_PREFIXLANG . "_" . $module_data. "_province WHERE alias like '". $array_op[1] ."'  ORDER BY weight ASC")->fetchColumn();
					}
					if($array_op[0] == 'map' and !empty($array_op[1]) and !empty($array_op[2]) and $id_tinhthanh > 0 )
					{
					    $id_quanhuyen = $db->query("SELECT districtid FROM ".NV_PREFIXLANG . "_" . $module_data. "_district WHERE provinceid =". $id_tinhthanh ." AND alias like '". $array_op[2] ."'")->fetchColumn();
					}
					if($array_op[0] == 'map' and !empty($array_op[1]) and !empty($array_op[2]) and !empty($array_op[3])  and $id_tinhthanh > 0 and $id_quanhuyen > 0)
					{
					    $id_xaphuong = $db->query("SELECT wardid FROM ".NV_PREFIXLANG . "_" . $module_data. "_ward WHERE  districtid =". $id_quanhuyen ." AND alias like '". $array_op[3] ."'")->fetchColumn();
					}
				}
				if($id_tinhthanh > 0)
